...
const trigrams = triBase.map(( [oldName, bin, name, char, unicodeChar], i) => ({
...
const pantherUrl = kingWen => {
    ...
    return `${base}bk1h${lo}-${hi}.html#${kingWen}`;
}
...
const iChing = hexagrams.map(([kingWen, bin, dec, oct, hexStr]) => ({ kingWen, bin, dec, oct, hexStr, binStr: bin.toString().padStart(6, "0"), }))
    .map(h => {
        const { binStr } = h;
        const upper = trigrams.find( t => t.binStr === binStr.slice(0, 3));
        const lower = trigrams.find( t => t.binStr === binStr.slice(3));
...
module.exports = {iChing, trigrams};